...
NATS_CREDS_FILE = Path.home() / ".config" / "nats" / "context" / "astral-prod-apps.creds"
...
def format_queues_output(raw_output: str) -> dict:
    ...
    incoming_class, incoming_color = get_queue_class(incoming_val)
    reader_class, reader_color = get_queue_class(reader_val)
    writer_class, writer_color = get_queue_class(writer_val)
    packer_class, packer_color = get_queue_class(packer_val)
    sender_class, sender_color = get_queue_class(sender_val)
